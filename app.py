# app.py - SSRA (combined risk prediction + preop recommender + monitoring + patient dashboard)
...
# -------------------------
# Patient input form (Module 1) - risk prediction
# -------------------------
...
if submitted:
    ...
    try:
        ...
        print("add_patient_record error:", e)
...
if run_monitor:
    try:
        ...
        if "patient_data" in st.session_state and st.session_state["patient_data"]:
            ...
            try:
                patient_data = {
                    ...
                    "lifestyle": {"smoker": bool(smoking) if 'smoking' in locals() else False, "alcohol_units_week": 0},
                    "surgery": {"type": str(surgery_type).lower() if 'surgery_type' in locals() else "", "subtype": "", "urgency": "emergency" if ('emergency' in locals() and int(emergency) == 1) else "elective"}
                }
            except Exception:
                patient_data = {"demographics": {"age": None, "sex": "", "bmi": None}, "comorbidities": {}, "medications": {}, "labs": {}, "lifestyle": {}, "surgery": {}}
        ...
        try:
            ...
            st.warning("Local DB helpers not available or failed; monitoring event not saved to DB.")
